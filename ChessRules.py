...
class ChessRules:
    def __init__(self, chessboard, from_coord, to_coord):
        ...
        for i, rule in enumerate(rules):
            if rule() is False:
                self.legality = False
                print(f"Rule {i+1} was not fulfilled")
    ...
    def basic_rules(self):
        ...
        if self.moving_piece is empty_square:
            return False
        if self.moving_piece.colour != self.ctmm:
            return False
    ...
    def not_in_check(self):
        ...
        self.make_move()
        opposite_colour_pieces = list(filter(lambda piece: piece.colour != self.ctmm, self.chessboard.pieces_on_board))                                     # Finds all enemy pieces on board
        king_position = next(filter(lambda piece: piece.char.upper() == 'K' and piece.colour == self.ctmm, self.chessboard.pieces_on_board)).position       # Locates friendly king position
        checking_pieces = list(filter(lambda enemy_piece: king_position in enemy_piece.legal_moves, opposite_colour_pieces))                                # Extracts all pieces checking friendly king
        self.revert_move()
